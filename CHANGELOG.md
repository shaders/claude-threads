...
### Added
- **The arbiter now watches the whole review chain, not just its own agent.** An MR that nobody was asked to review, a reviewer that never showed up, a clean review with no approval in GitLab, a result never handed back to the requester, a human never told the task finished — five steps that span two bots on two hosts, each with exactly one party able to perform it. The chain records who owes what and chases it in the only way that reaches that party: our own agent gets an injected instruction, another bot gets a mention in the same thread (the one thing that wakes its session), a person gets an @mention once reminders are spent. Decisions live in a pure reducer (`arbiter/chain/reducer.ts`), so a five-step cross-process flow is testable from literals. **Closure is an event, not a deadline:** our turn ending, the reviewer posting, a `glab`-confirmed approval. Timers are only a missing-owner detector, measured as a sliding silence window from the owner's last sign of life — so a reviewer visibly working on a long diff is never interrupted, while one whose process died is noticed in about two minutes instead of half an hour. Two cases skip the ladder because nudging cannot work: a teammate whose bot announced a rate limit in the thread, and a reviewer who holds no session in this channel at all (the case review-ping used to swallow silently). One haiku call in the whole chain, classifying whether a finished review demanded changes — because that alone decides whether an approval is owed. `!arbiter` lists what a thread is waiting on; `health.json` gained `chainOpen`/`chainStuck` so a stuck chain shows on the status board. Configure with `arbiterChain`; the reviewer's name comes from `reviewPing.botName`.
- **Post edits are a liveness signal.** A claude-threads bot mid-task rewrites one rolling tool line instead of posting again, so on the websocket a teammate hard at work looked exactly like a teammate whose process had died — and any watchdog watching for silence had to be given a window long enough to be useless. `post_edited` is now emitted as its own platform event, deliberately never routed to an agent as input (a rewritten line would replay as a new instruction).

### Fixed
- **Every observer of tool traffic finally sees it on Claude sessions.** Claude CLI reports tool calls as `tool_use` blocks inside `assistant` messages and outcomes as `tool_result` blocks inside `user` messages; only the Codex backend synthesizes the standalone events. Everything watching tool traffic keys off that standalone shape, so on a Claude session the arbiter's delivery ledger, docs-ping, return-address, the teammate observer and the bug-report tool list all observed *nothing*: an obligation could only ever be closed by a delivery the bot itself made, and the arbiter went on reminding agents about deliveries they had just performed — the loop that posted the same handoff every three minutes for half an hour. `operations/events/tool-events.ts` derives the standalone events from the block-carrying ones and both pre- and post-processing fan them out through one `notifyToolObservers()`. Deliberately not emitted at the backend: the transformer has branches for `assistant` blocks *and* for standalone tool events, so synthetic events reaching MessageManager would render every tool line twice. Failed calls stay failed — an obligation is fulfilled only by a `tool_result` without `is_error`, which is what keeps a refused `send_to_teammate` from counting as delivered.
- **Launching an agent no longer breaks the rolling tool line, and gets its own post back.** Claude CLI renamed the subagent tool `Task` → `Agent`; only the old name was recognised, so every launch fell through to the generic-unknown-tool renderer as `● **Agent**`. Two things followed. The subagent post — prompt, live elapsed time, 🔽 toggle — stopped appearing at all, leaving `SubagentExecutor` unreachable. And because *any* content ends a run of same-tool calls, each launch cut the rolling line in half: a fan-out that should read `💻 Bash ×40` came out as `×17`, `×14`, `×4`, `×5`, every one of them stranded on ⏳ because nothing ever closed them either. Both names are accepted now. Verified against 230 real `Agent` calls in the thread logs and zero `Task` ones — note the CLI still *advertises* `Task` in its init event, so the init tool list is not a safe place to look.
- **A tool's outcome finally reaches the line that is waiting for it.** Claude CLI reports outcomes as `tool_result` blocks inside `user` messages, not as the `tool_result` events the Codex backend synthesizes, and `user` fell through to `default: []` — so `takeToolOutcome`'s whole path was Codex-only and every rolling line kept its ⏳ for the rest of the session (`💻 Bash ×8 glab ci status ✓ (41s)`, the documented shape, was never once rendered on Claude). Two outcomes are picked up: the rolling line's status, and the subagent post whose launch is still open. Deliberately *not* the `↳ ✓` line the Codex path adds under every other tool — a second line under every `Edit`, `Write` and MCP call is the wall of text the collapsing exists to remove, and no Claude thread has ever shown one. No extra flush is emitted either: the status rides the existing debounced flush, because an in-flight flush makes the next rewrite stand down and open a fresh line, which is the very shattering being fixed.
- **A background agent no longer rewrites its post every 5s until the session ends.** `run_in_background` returns its tool_result within milliseconds — "agent launched successfully" — nowhere near the agent's actual end, so treating that ack as completion would stamp `✅ 0s` on an agent with three minutes left to run, and *not* treating it as one leaves the elapsed-time timer running forever over an end that is never observed. Background launches are therefore not registered for completion and their post says `🚀 running in background` instead of counting seconds that mean nothing. Foreground launches are unaffected and still show real elapsed time.
